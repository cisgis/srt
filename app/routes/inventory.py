...
STATUS_OPTIONS = [
    ...
    "Sold", "Damaged", "In Repair", "Retired / Decommissioned", "Lost"
]

@router.get("/", response_class=HTMLResponse)
def inventory_list(request: Request, search: str = "", status: str = "", location: str = ""):
    db = get_db()
    ...
    return templates.TemplateResponse("inventory/list.html", {
        "request": request, "products": products, "vendors": vendors,
        "status_options": STATUS_OPTIONS, "search": search,
        "filter_status": status, "filter_location": location,
    })
...
def inventory_new(request: Request):
    ...
    return templates.TemplateResponse("inventory/form.html", {
        ...
        "vendors": vendors, "status_options": STATUS_OPTIONS,
    })
...
async def inventory_create(
    ...
    cost_price: str = Form(""), resale_price: str = Form(""),
    list_price: str = Form(""), purchase_date: str = Form(""),
    resale_date: str = Form(""), recertification_date: str = Form(""),
    certification_expiration_date: str = Form(""),
    weight: str = Form(""), dimensions: str = Form(""),
    status: str = Form("Available"), location: str = Form(""),
    vendor_id: str = Form(""),
    ...
    if mtr_file and mtr_file.filename:
        ...
        with open(dest, "wb") as f: shutil.copyfileobj(mtr_file.file, f)
    if drawing_file and drawing_file.filename:
        ...
        with open(dest, "wb") as f: shutil.copyfileobj(drawing_file.file, f)

    db = get_db()
    db.execute("""INSERT INTO Product VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
        parts_number, product_service_description,
        _f(cost_price), _f(resale_price), _f(list_price),
        purchase_date or None, resale_date or None,
        recertification_date or None, certification_expiration_date or None,
        mtr_fn, drawing_fn, _f(weight), dimensions or None,
        status,
        # FIX (Bug 8): total_cost_of_ownership was always hardcoded to None.
        # Now seeded from cost_price so it has a meaningful starting value.
        # You can add a dedicated form field later if you need to track
        # accumulated costs (repairs, recertification, etc.) separately.
        _f(cost_price),
        location or None, vendor_id or None,
    ))
    db.commit(); db.close()
...
def inventory_detail(request: Request, parts_number: str):
    ...
    vendors = db.execute("SELECT * FROM Vendor ORDER BY vendor_name").fetchall()
    # FIX (Bug 2): Was INNER JOIN Clients which silently dropped any transaction
    # linked to a packing slip with a NULL client_id. Changed to LEFT JOIN so
    # all external transactions appear in the history regardless.
    ext_txns = db.execute("""
    ...
    return templates.TemplateResponse("inventory/detail.html", {
        ...
        "vendors": vendors, "status_options": STATUS_OPTIONS,
        "ext_txns": ext_txns, "int_txns": int_txns,
    })
...
async def inventory_edit(
    ...
    def _f(v): return float(v) if v else None
    db = get_db()
    existing = db.execute("SELECT * FROM Product WHERE parts_number=?", (parts_number,)).fetchone()

    mtr_fn    = existing["mtr_filename"]
    drawing_fn= existing["drawing_filename"]
    if mtr_file and mtr_file.filename:
        mtr_fn = f"mtr_{parts_number}_{mtr_file.filename}"
        with open(config.UPLOAD_DIR / mtr_fn, "wb") as f: shutil.copyfileobj(mtr_file.file, f)
    if drawing_file and drawing_file.filename:
        drawing_fn = f"drw_{parts_number}_{drawing_file.filename}"
        with open(config.UPLOAD_DIR / drawing_fn, "wb") as f: shutil.copyfileobj(drawing_file.file, f)
    ...
    db.execute("""UPDATE Product SET
        ...
        WHERE parts_number=?""", (
        product_service_description, _f(cost_price), _f(resale_price), _f(list_price),
        purchase_date or None, resale_date or None,
        recertification_date or None, certification_expiration_date or None,
        mtr_fn, drawing_fn, _f(weight), dimensions or None,
        status, location or None, vendor_id or None, parts_number,
    ))
    db.commit(); db.close()
...
def serve_file(filename: str):
    # FIX (Bug 1): Original code did not sanitize `filename` at all, allowing
    # path traversal attacks like /inventory/file/../../config.py which would
    # serve sensitive files from outside the upload directory.
    # Now we resolve the final path and confirm it still lives inside UPLOAD_DIR.
