...
async def quote_create(
    ...
    items_json_raw = form.get("items_json", "[]")
    if hasattr(items_json_raw, "filename"):
        items_json = "[]"
    else:
        items_json = str(items_json_raw) if items_json_raw else "[]"
    try:
    ...
    for it in items:
        ...
        try:
            ...
            qty = 1
        db.execute(
            """INSERT INTO Quote_Items (quote_number,parts_number,quantity,quoted_price,lead_time)
                      VALUES (?,?,?,?,?)""",
