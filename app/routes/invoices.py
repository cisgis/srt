...
def inv_list(request: Request):
    ...
    """).fetchall()
    db.close()
    return templates.TemplateResponse("invoices/list.html", {"request": request, "invs": invs})
...
def inv_new(request: Request, from_pl: str = ""):
    ...
    if from_pl:
        prefill_pl = db.execute("SELECT * FROM Packing_Slip WHERE packing_slip_number=?", (from_pl,)).fetchone()
        if prefill_pl and prefill_pl["client_id"]:
            prefill_client = db.execute("SELECT * FROM Clients WHERE client_id=?", (prefill_pl["client_id"],)).fetchone()
        if prefill_pl and prefill_pl["quote_number"]:
            prefill_quote = db.execute("SELECT * FROM Quote WHERE quote_number=?", (prefill_pl["quote_number"],)).fetchone()
    db.close()
    today = datetime.today().strftime("%Y-%m-%d")
    return templates.TemplateResponse("invoices/form.html", {
        "request": request, "pls": pls, "payment_terms": PAYMENT_TERMS,
        "today": today, "prefill_pl": prefill_pl,
        "prefill_client": prefill_client, "prefill_quote": prefill_quote,
    })
...
async def inv_create(
    ...
    db.execute("INSERT INTO Invoice VALUES (?,?,?,?,?,?)", (
        invnum, packing_slip_number, purchase_number or None,
        payment_term or None, invoice_date, client_id or None,
    ))
    db.commit(); db.close()
...
def inv_detail(request: Request, inv_number: str):
    db = get_db()
    inv = db.execute("SELECT * FROM Invoice WHERE invoice_number=?", (inv_number,)).fetchone()
    pl  = db.execute("SELECT * FROM Packing_Slip WHERE packing_slip_number=?", (inv["packing_slip_number"],)).fetchone() if inv and inv["packing_slip_number"] else None
    client = db.execute("SELECT * FROM Clients WHERE client_id=?", (inv["client_id"],)).fetchone() if inv and inv["client_id"] else None
    quote = None
    items = []
    if pl and pl["quote_number"]:
        quote = db.execute("SELECT * FROM Quote WHERE quote_number=?", (pl["quote_number"],)).fetchone()
        items = db.execute("""
    ...
    return templates.TemplateResponse("invoices/detail.html", {
        "request": request, "inv": inv, "pl": pl,
        "client": client, "quote": quote, "items": items,
...
async def inv_update_po(inv_number: str, purchase_number: str = Form(...)):
    db = get_db()
    db.execute("UPDATE Invoice SET purchase_number=? WHERE invoice_number=?", (purchase_number, inv_number))
    db.commit(); db.close()
...
def inv_pdf(inv_number: str):
    ...
    if inv.get("packing_slip_number"):
        row = db.execute("SELECT * FROM Packing_Slip WHERE packing_slip_number=?", (inv["packing_slip_number"],)).fetchone()
        if row: pl = dict(row)
    client, quote, items = {}, {}, []
    if inv.get("client_id"):
        row = db.execute("SELECT * FROM Clients WHERE client_id=?", (inv["client_id"],)).fetchone()
...
async def inv_send(
    ...
    db.close()
    pdf = pdf_service.build_invoice_pdf(inv, pl, client, items, quote)
    result = email_service.send_document_email(to_email, subject, body, pdf, f"{inv_number}.pdf")
