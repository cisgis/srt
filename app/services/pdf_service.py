...
Uses the client's example PDFs as backgrounds and overlays variable text.
"""
...
QUOTE_TEMPLATE = TEMPLATE_DIR / "SRTGN - Estimate 20260307-2 C.pdf"
PL_TEMPLATE = TEMPLATE_DIR / "SRTGN - PL 20260307-2 C.pdf"
...
def build_quote_pdf(quote: dict, client: dict, items: list) -> bytes:
    ...
    # ── Header: Logo + Company Name + Doc Type ──────────────
    logo_path = config.LOGO_PATH
    if logo_path.exists():
        c.drawImage(
            str(logo_path), 127, 710, width=69, height=22, preserveAspectRatio=True
        )

    c.setFont("Helvetica-Bold", 18)
    c.drawString(202, 720, "STEEL RIVER TECHNOLOGIES")

    c.setFont("Helvetica", 16)
    c.drawString(473, 720, "Sales Quote")

    # ── Company Address (left) ──────────────────────────────
    c.setFont("Helvetica", 9)
    c.drawString(27, 650, "8810 E CR-95")
    c.drawString(27, 636, "Midland, TX 79706")
    c.drawString(27, 620, "Phone: 432-618-0169")
    c.drawString(27, 604, "Email: contact@steelriver-tech.com")
    ...
    ship_to_name = client.get("customer_name", "")
    well = client.get("well_name", "") or quote.get("ship_to", "")
    if well:
        ship_to_name = f"{ship_to_name} {well}".strip()

    _draw_box(c, 28, 550, 60, 34, "Ship From", [quote.get("ship_from", "")])
    _draw_box(c, 385, 550, 175, 34, "Ship To", [ship_to_name])

    # ── Meta Row: Date | Quote No. | Expiration | Term ───────
    _draw_meta_row(
        c,
        486,
        [
            ("Date", _format_date(quote.get("quote_date", ""))),
            ("Quote No.", _strip_doc_prefix(quote.get("quote_number", ""))),
            ("Expiration Date", _format_date(quote.get("quote_expiration_date", ""))),
    ...
    subtotal = sum(
        (it.get("quoted_price", 0) or 0) * (it.get("quantity", 1) or 1) for it in items
    )
